...
It is importnant to note that not all assumptions necessary for SLR are not met with our data:
...
Our data immediately contrasts our hypothesis by showing a positive relationship between CO$_{2}$ emissions and life expectancy, as opposed to a negative relationship like we expected (Figure 1, Figure 2).
```{r}
#| fig-cap: "Figure 1."
#| fig-align: center
...
ggplot(aes(x = .fitted, y = .resid)) +
  ...
  theme_bw()
```
To assess the quality of this model, we can compare the variation in the observed life expectancy values to the variation in its expected life expectancy values.
...
Figure 4 shows 42.41% of variability in life expectancy is explained by our linear regression model using CO$_{2}$ emissions as a predictor ($14.02/33.04 = 0.4241$).

#### Simulation

We simulated data to further asses the quality of our linear model by comparing them to the observed data.
```{r}
#| layout-ncol: 2
#| fig-cap: "Figure 6."
#| fig-align: center
...
tibble(sims = sims_r) |>
  ...
  theme_bw()
```
The $R^{2}$ values for simulations based on our model tend to be centered around 0.1801 (Figure 7).
...
Similarly, since the $R^{2}$ value of the observed average life expectancies is relatively small, only about `r round(median(sims_r), 4)`, we can conclude that our simulated linear regression model does not match our observed data very well. Thus, it is further supported that our SLR model and does not have very strong predictive ability. 
